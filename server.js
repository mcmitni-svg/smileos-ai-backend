...
const AGENTS = [
  ...
  },
  {
    ...
    tasks: [
      "Review de l'architecture technique",
      'Optimisation des performances',
      'Identification des améliorations produit',
  ...
  },
  {
    ...
    tasks: [
      ...
      'Optimisation SEO',
      'Analyse du profil client idéal',
...
async function runAgent(agentId) {
  ...
  try {
    const message = await anthropic.messages.create({
      ...
      messages: [{
        role: 'user',
        content: `Tu es un agent IA autonome spécialisé en "${agent.role}" pour SmileOS, une startup innovante.
Tâche : ${task}
Produis un résultat professionnel, concret et actionnable en français (4-5 phrases).
Explique ce que tu as fait et ce que ça apporte à SmileOS.
Parle simplement, comme un conseiller qui s'adresse au fondateur.
Commence directement par le résultat, sans formule de politesse.`
      }]
    });
    const result = message.content[0].text;
    if (pool) {
      await pool.query(
        'INSERT INTO agent_results (agent_id, agent_name, agent_emoji, task, result) VALUES ($1, $2, $3, $4, $5)',
        [agent.id, agent.name, agent.emoji, task, result]
      );
    }
...
// ⏰ Cron : agents toutes les 2h en décalé
...
initDB().then(() => {
  ...
  app.listen(PORT, () => {
    ...
    console.log(`🤖 4 agents actifs — mise à jour automatique toutes les 2h\n`);
  });
});
